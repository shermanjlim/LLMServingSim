...
def _device_allocate_policy(self, ev, kv_bytes, npu_free_bytes, hbf_free_bytes):
    ...
    """
    # Identify high-reuse blocks for NPU placement
    prefix_depth = len(ev.full_token_ids) - len(ev.token_ids)
    
    # Highest priority: blocks with proven reuse
    if ev.hit_count > 0 and kv_bytes <= npu_free_bytes:
        return Device.NPU
    
    # High priority: very deep shared prefixes
    if prefix_depth > 18 and kv_bytes <= npu_free_bytes:
        return Device.NPU
    
    # Medium priority: short prompts with strict capacity limits
    if (ev.num_input_tokens < 250 and self.npu_used < 0.7 * self.npu_mem 
        and kv_bytes <= npu_free_bytes):
        return Device.NPU
    
    # Low reuse likelihood - prefer HBF
    if kv_bytes <= hbf_free_bytes:
        return Device.HBF
    
    # Fallback to NPU if HBF is full
    return Device.NPU
# EVOLVE-BLOCK-END
