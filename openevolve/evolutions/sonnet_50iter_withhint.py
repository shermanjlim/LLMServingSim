...
def _device_allocate_policy(self, ev, kv_bytes, npu_free_bytes, hbf_free_bytes):
    ...
    """
    SHORT_PREFILL_THRESHOLD = 1024  # tokens; tune as needed

    is_prefill_block = len(ev.full_token_ids) <= ev.num_input_tokens
    is_short_prefill = ev.num_input_tokens <= SHORT_PREFILL_THRESHOLD

    if is_prefill_block and is_short_prefill and kv_bytes <= hbf_free_bytes:
        return Device.HBF
